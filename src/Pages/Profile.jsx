...
const Profile = () => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      try {
        const res = await fetch(`https://hobby-hub-server-site.vercel.app/users/${user.email}`);
  ...
  useEffect(() => {
    const fetchUserGroups = async () => {
      ...
      setGroupsError("");
      try {
        const res = await fetch(`https://hobby-hub-server-site.vercel.app/mygroups/${encodeURIComponent(user.email)}`);
        const data = await res.json();
